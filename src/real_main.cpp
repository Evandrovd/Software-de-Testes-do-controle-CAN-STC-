...
#define vel_min -100 // Velovcidade minima que estaremos aceitando para o motor neste momento;
#define vel_max 100 // Velocidade maxima que estaremos aceitando para o motor neste momento;

#define CAN_TX 5 // Pino padrao de envio can na esp32;
#define CAN_RX 4 // Pino padrao de recepcao can na esp32;

#define delay_inversao 1 // tempo para esperar quando existe e pedida uma inversao do motor;
...
// variavel de contagem do buffer para entrada de comandos; Utilizada somente na funcao handle_incoming_messages().
...
void handle_incoming_messages() {
  ...
  if (ESP32Can.readFrame(rxFrame, 0)) { 
        ...
        // Se for uma mensagem de anuncio do motor.
        if (rxFrame.data[0] == 0x55 && rxFrame.data[3] == 0x55 && rxFrame.data[7] == 0x55) 
          { // se o frame 0,3 e 7 contem 0x55 assumimos que tudo e 0x55 e logo e uma mensagem de anuncio do motor;
            life_signal = 0; // Zera o sinal de vida pra que a proxima mensagem seja o numero 1;
      ...
      // Se nao for uma mensagem de anuncio de motor.
        ...
        { 
          ...
          txFrame.data[4] = 0b00000001; // byte 4;  O primeiro bit controla se o motor esta ligado ou nao, o segundo se o controle por torque ou velocidade, o resto n serve p nada;\_que se  traduz para o numero e escrito de traz para frente;
...
void set_if_valid_speed(int l_speed) { 
  ...
  if (l_speed >= vel_min && l_speed <= vel_max ) { 
    ...
    int new_speed = l_speed + 32000;
    // se houver inversao de sentido;
    if (((speed > 32000) && (new_speed < 3200)) || ((speed  < 32000) && (new_speed > 32000))) {
      // seta a velocidade do motor para 0;
      speed = 0;
    ...
    // Se o sentido novo do kovimento for positivo;
    if (speed > 32000) {
      // seta a corrente de fase para o positiva;
      current_phase = 32000 + 1;
...
void handle_set_speed_delays() {
  if ((millis() - last_inversion_time) >= delay_inversao) {
    speed = delayed_speed;
  }
}
